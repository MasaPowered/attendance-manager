    ...
    <form method="POST" action="{{ route('admin.work_reports.edit_done') }}">
        ...
        <?php if ($message_array->arrivalcheck) : ?>
            <?php if (!empty($message_array->arrivaltime)) : ?>
                <input type="time" name="arrivaltime" step="0.001" value="<?php echo substr($message_array->arrivaltime, 0, 8); ?>">
            <?php else : ?>
                <input type="time" name="arrivaltime">
            <?php endif; ?>
            <br>
            遅刻の場合遅刻時間入力：
            <input id="latetime" type="time" name="latetime" step="0.001" value="<?php echo substr($message_array->latetime, 0, 8); ?>" <?php if (!$message_array->arrivalcheck) echo "disabled"; ?>>
            <br>
            出勤業務内容：<br>
        ...
        <?php if ($message_array->leavecheck) : ?>
            <?php if (!empty($message_array->leavetime)) : ?>
                <input type="time" name="leavetime" step="0.001" value="<?php echo substr($message_array->leavetime, 0, 8); ?>">
            <?php else : ?>

...
<?php if (!empty($message_array)) : ?>
    <form method="POST" action="report_edit_done">
        @csrf
        ...
        <?php if ($message_array->arrivalcheck) : ?>
            <?php if (!empty($message_array->arrivaltime)) : ?>
                <input type="time" name="arrivaltime" value="<?php echo $message_array->arrivaltime; ?>">
            <?php else : ?>
                <input type="time" name="arrivaltime">
            <?php endif; ?>
            <br>
            遅刻の場合遅刻時間入力：
            <input id="latetime" type="time" name="latetime" value=<?php echo $message_array->latetime; ?> <?php if (!$message_array->arrivalcheck) echo "disabled"; ?>>
            <br>
            出勤業務内容：<br>
        ...
        <?php if ($message_array->leavecheck) : ?>
            <?php if (!empty($message_array->leavetime)) : ?>
                <input type="time" name="leavetime" value="<?php echo $message_array->leavetime; ?>">
            <?php else : ?>

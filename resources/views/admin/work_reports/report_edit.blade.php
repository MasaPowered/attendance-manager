    ...
    <form method="POST" action="{{ route('admin.work_reports.edit_done') }}">
        ...
        <?php if ($message_array->arrivalcheck) : ?>
            <?php if (!empty($message_array->arrivaltime)) : ?>
                <input type="time" name="arrivaltime" step="1" value="<?php echo date('H:i:s', strtotime($message_array->arrivaltime)); ?>">
            <?php else : ?>
        ...
        <?php if ($message_array->leavecheck) : ?>
            <?php if (!empty($message_array->leavetime)) : ?>
                <input type="time" name="leavetime" step="1" value="<?php echo date('H:i:s', strtotime($message_array->leavetime)); ?>">
            <?php else : ?>
